...
table_get(Env0,_t,_i,_t2) :- get(_t,_x,_t2).
...
table_env(X) :- new(X0),set(X0,"toListValues",table_toListValues,X1),set(X1,"toListKeys",table_toListKeys,X2),set(X2,"set",table_set,X3),set(X3,"get",table_get,X4),X=X4.